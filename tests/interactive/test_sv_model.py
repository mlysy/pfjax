...
from pfjax.experimental.models.stovol_model import StoVolModel as StoVolModel_test
from pfjax.particle_resamplers import resample_multinomial, resample_mvn
from pfjax.test.models.stovol_model import StoVolModel

# We will be giving two versions of sde model, one from standard definition and one from experiement
# Using the experimental sde model will enable the use of particle_filter_rb
...
step_out = sv_model.pf_step(
    key=key,
    x_prev=state[0],
    y_curr=meas[1],
    theta=true_theta,
)
step_out_test = sv_model_test.pf_step(
    key=key,
    x_prev=state[0],
    y_curr=meas[1],
    theta=true_theta,
...
print(jnp.min(jnp.abs(step_out[0] - step_out_test[0])))
print(jnp.min(jnp.abs(step_out[1] - step_out_test[1])))


# --- check loglikelihood calculation ------------------------------------------
...
pf_out_test = pf.particle_filter(
    model=sv_model_test,
    key=key,
    y_meas=meas,
    theta=true_theta,
