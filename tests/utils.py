...
def lv_setup(self):
    ...
    tau_H = 0.25
    tau_L = 0.35
    self.theta = jnp.array([alpha, beta, gamma, delta, sigma_H, sigma_L, tau_H, tau_L])
    # data specification
    dt = 0.09
    n_res = 3
    self.model_args = {"dt": dt, "n_res": n_res}
    self.n_obs = 7
    self.x_init = jnp.block(
        [[jnp.zeros((n_res - 1, 2))], [jnp.log(jnp.array([5.0, 3.0]))]]
    )
    self.n_particles = 25
...
def test_loglik_full_models(self):
    ...
    loglik1 = pf.loglik_full(model1, y_meas, x_state, theta)
    # joint loglikelihood with inherited class
    loglik2 = pf.loglik_full(model2, y_meas, x_state, theta)
    self.assertAlmostEqual(rel_err(loglik1, loglik2), 0.0)
...
def test_sde_state_sample_for(self):
    ...
    n_obs = self.n_obs
    n_particles = self.n_particles
    model1 = self.Model(**model_args)  # lax.scan version
    model2 = self.Model2(**model_args)  # for-loop version
...
def test_sde_state_lpdf_for(self):
    ...
    n_obs = self.n_obs
    n_particles = self.n_particles
    model1 = self.Model(**model_args)  # vmap version
    model2 = self.Model2(**model_args)  # for-loop version
    # generate previous timepoint
    key, subkey = random.split(key)
    x_prev = self.x_init
