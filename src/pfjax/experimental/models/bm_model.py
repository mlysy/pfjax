...
from pfjax.experimental.base_model import BaseModel
...
class BMModel(BaseModel):
    ...
    Args:
        ...
        unconstrained_theta: Whether or not to use the regular parameters scale
            `theta = (mu, sigma, tau)` or the unconstrained scale
            `theta = (mu, log(sigma), log(tau))`.
    """
    ...
    def state_sample(self, key, x_prev, theta):
        ...
        return x_mean + x_sd * jax.random.normal(key=key)
    ...
    def meas_lpdf(self, y_curr, x_curr, theta):
        ...
        if self._unconstrained_theta:
            theta = self._constrain_theta(theta)
        tau = theta[2]
        return jnp.squeeze(stats.norm.logpdf(y_curr, loc=x_curr, scale=tau))
    ...
    def meas_sample(self, key, x_curr, theta):
        ...
        if self._unconstrained_theta:
            theta = self._constrain_theta(theta)
        tau = theta[2]
        return x_curr + tau * jax.random.normal(key=key)
