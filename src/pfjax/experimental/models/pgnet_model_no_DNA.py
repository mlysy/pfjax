...
class PGNETModelNoDNA(sde.SDEModel):
    ...
    def _expit(self, x):
        ...
        return jnp.append(jnp.exp(x[:3]), x4) + self._eps
    ...
    # def _drift(self, x, theta):
    #     """
    #     OLD: Calculate the drift on the original scale.
    #     """
    #     mu1 = theta[2]*x[3] - theta[6]*x[0]
    #     mu2 = 2*theta[5]*x[2] - theta[7]*x[1] + \
    ...
    def meas_lpdf(self, y_curr, x_curr, theta):
        ...
        return jnp.sum(
            ...
            jsp.stats.norm.logpdf(y_curr, loc=self._expit(x_curr[-1])[..., :3], scale=tau)
        )
    ...
    def meas_sample(self, key, x_curr, theta):
        ...
        return self._expit(x_curr[-1])[..., :3] + tau * random.normal(key, (self._n_state[1]-1,))
    ...
    def pf_init(self, key, y_init, theta):
        ...
        **FIXME:** Explain what the proposal is and why it gives `logw = 0`.
        ...
        x_init123 = jnp.log(y_init + tau * random.truncated_normal(
            subkey,
            lower=-y_init/tau,
            upper=jnp.inf,
            shape=(self._n_state[1]-1,)
        ))
        x_init4 = self._logit(dna0)
    ...
    def pf_step(self, key, x_prev, y_curr, theta):
        ...
        if self._bootstrap:
            ...
            x_curr, logw = self.bridge_prop(
                ...
                logy_curr, jnp.eye(4)[:-1, :], jnp.diag(omega)
            )
        return x_curr, logw
...
class PGNETModelNoDNA_OLD(sde.SDEModel):
