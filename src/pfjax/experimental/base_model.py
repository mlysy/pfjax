class BaseModel(object):
    ...
    **Notes:**
    ...
    - `additive_gaussian_meas` and `fixed_init` create methods at runtime, so that no methods are created if these are set to `False`.  However, in order for the user to be able to "overwrite" these at definition time, the methods are created conditionally.
    ...
    Args:
        ...
        fixed_init: Boolean for whether the initial state variable is fixed.

    """

    def __init__(self, bootstrap, additive_gaussian_meas, fixed_init):
