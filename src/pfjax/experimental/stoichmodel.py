...
class StoichModel(sde.SDEModel):
    ...
    def _Hazard(self, x, param):
        ...
        def h(x, param_i, i):
            ...
            ans = jnp.where(n_mols==0, param_i, 
                  jnp.where(n_mols==1, param_i * x[index[0][0]], 
                  jnp.where((n_mols==2)&(n_type==1), param_i * x[index[0][0]] * (x[index[0][0]]-1) / 2,
                  jnp.where((n_mols==2)&(n_type==2), param_i * jnp.prod(x[index[0]]), 0))))
