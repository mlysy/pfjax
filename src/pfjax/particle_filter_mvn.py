...
#     flat_particles = particles.reshape(n_particles, n_states) # flatten to calculate mu, cov
#     print("flat shape: ", flat_particles.shape)

#     # variables are the rows in jnp.cov()
#     mu = jnp.average(flat_particles, axis=0, weights=prob).reshape(-1, ) 
#     cov_mat = jnp.cov(jnp.transpose(flat_particles), aweights=prob)
...
#                                          cov=cov_mat,
#                                          shape=(n_particles, 1))
#     print("samples shape: ", samples.reshape(p_shape).shape)
#     return samples.reshape(p_shape), mu, cov_mat
...
# def particle_filter_mvn(model, y_meas, theta, n_particles, key):
#     """
...
#         resampled_particles = particle_resample_mvn(particles = carry["X_particles"], 
#                                                     logw=carry["logw"],
#                                                     key = subkey) 
#         prev_particles = resampled_particles[0]
...
#         )(prev_particles, jnp.array(subkeys))
