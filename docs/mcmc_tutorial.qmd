---
title: "Bayesian Inference with PFJAX"
author: 
  name: "Michelle Ko, Martin Lysy"
...
In the [Introduction](pfjax.qmd) to **PFJAX**, we saw how to set up a model class for a state-space model and how to use a particle filter to estimate its marginal likelihood 
...
where the model parameters are $\tth = (\mu, \log \sigma, \log \tau)$.  The details of setting up the appropriate model class are provided in the [Introduction](pfjax.qmd).  Here we'll use the version of this model provided with **PFJAX**: `pfjax.models.BMModel`.
